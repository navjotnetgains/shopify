...
import path from "path";
import fs from "fs/promises";
import db from "../db.server"; // adjust path based on your project structure

// 🚀 Remix action to handle POST from app proxy form
export const action = async ({ request }) => {
  const formData = await request.formData();
  const customerId = formData.get("customerId");
  const name = formData.get("name");
  ...
  try {
    ...
    // ✅ Return success response for snippet JavaScript
    return json({ success: true, message: "Your gallery upload is in process." });
  } catch (error) {
...
// ➡️ No frontend loader/export default needed here if it's purely backend API endpoint
export default function UploadGalleryRoute() {
  return null;
}
